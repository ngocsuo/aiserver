        ...
        try:
            ...
            try:
                ...
                if 'APIError(code=-1130)' in str(e):
                    ...
                    # Thông báo cho người dùng - sẽ hoạt động tốt khi triển khai trên server riêng
                    logger.warning("Geographic restriction detected. This will work on your private server with direct connection.")
                ...
                # Không cần xử lý proxy nữa, đã loại bỏ hoàn toàn
                logger.warning("Proxy handling has been removed. Using direct connection only.")
        ...
        try:
            ...
            if start_date is not None:
                ...
                while chunk_start < end_timestamp:
                    ...
                    # Especially for 1m data which has more candles, but reduced delay further
                    if timeframe == "1m":
                        time.sleep(0.2)  # 0.2 second between requests for 1m data (giảm từ 1 xuống 0.2)
                    else:
                        time.sleep(0.2)  # 0.2 seconds for other timeframes (giảm từ 0.5 xuống 0.2)
