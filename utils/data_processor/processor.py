...
class DataProcessor:
    ...
    def split_train_val_test(self, data=None, target_col='target_class'):
        ...
        try:
            ...
            test_size = config.TEST_SIZE / (config.VALIDATION_SIZE + config.TEST_SIZE)
    ...
    def prepare_latest_cnn_data(self, data, lookback=config.SEQUENCE_LENGTH):
        ...
        try:
            ...
            # Create image-like data using the feature engineer
            X_image, _ = self.feature_engineer.create_image_data(
                recent_data, 
                with_target=False,
                seq_length=lookback
            )
            
            logger.info(f"Latest CNN data prepared for prediction")
