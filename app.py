...
if st.session_state.selected_tab == "Live Dashboard":
    ...
    # Import module hiển thị trạng thái huấn luyện
    import training_status_tab
    
    # Render tab trạng thái huấn luyện
    training_status_tab.render_training_status_tab()
    
    # Xử lý sự kiện khi người dùng yêu cầu huấn luyện mô hình
    if "training_requested" in st.session_state and st.session_state["training_requested"]:
        st.session_state["training_requested"] = False
        train_models()  # Gọi hàm huấn luyện mô hình
