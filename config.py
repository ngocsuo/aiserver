...
ALL_TIMEFRAMES = ["1m", "5m", "15m", "1h",
                  "4h"]  # Tất cả khung thời gian hỗ trợ
...
TRADING_SETTINGS = {
    ...
    ["1m", "5m"],  # Các khung thời gian có thể sử dụng cho bot
    "default_timeframe": "5m",  # Khung thời gian giao dịch mặc định
    "min_price_movement": 0.0,  # Biến động giá tối thiểu (USDT) để vào lệnh
    "default_min_price_movement":
    6.0,  # Giá trị mặc định cho biến động giá tối thiểu
    "update_interval": {
...
DATA_RANGE_OPTIONS = {
    "realtime":
    3,  # 3 ngày dữ liệu cho biểu đồ thời gian thực (giảm để load nhanh hơn)
    "training":
    90,  # 3 tháng (90 ngày) dữ liệu cho huấn luyện mặc định (giảm để training nhanh hơn)
...
today = datetime.now()
DEFAULT_TRAINING_START_DATE = (
    today -
    timedelta(days=DATA_RANGE_OPTIONS["training"])).strftime("%Y-%m-%d")
